...
import { functor, isDefined } from "react-stockcharts/lib/utils";
...
function getBars(props, moreProps) {
	...
	// props.widthRatio = 1.0;

	const width = widthFunctor(Object.assign({}, props, {widthRatio:1.0}), {
		xScale,
		xAccessor,
		plotData
	});
	console.log("getWidth", width, plotData, props)
	console.log("widthFunctor", widthFunctor)
	console.log("xScale", xScale, "range", xScale.range())
	console.log("xAccessor", xAccessor)
	console.log("yAccessor", yAccessor)
	/*
	...
	*/
	...
	const bars = plotData
		...
		.map(d => {
			...
			console.log('bars', d, xScale(xAccessor(d)), offset)
